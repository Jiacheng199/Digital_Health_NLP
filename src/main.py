...
def process():
    ...
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    pending_check = False
    map = map_sys('uploads/'+userid+'/'+fileid+'.txt', 'process/'+userid+'/'+fileid+'.csv', pending_check)
    res,pending_check = map.mapping()
    cursor.execute('INSERT INTO Mappings(id,user_id,Editdate) VALUES(%s, %s,%s)', (fileid, userid, time.strftime('%Y-%m-%d %H:%M:%S')))
    con.commit()
    os.remove(os.path.join(app.config["UPLOAD_FOLDER"], userid, fileid+'.txt'))
    cursor.execute('SELECT * FROM Mappings WHERE id=%s', fileid)
    result = cursor.fetchone()
    if result:
        return jsonify({'message': 'Process successfully'}), 200

@app.route("/getmaps",methods=["GET"])
def getmap():
    userid = request.headers.get('Getmapping').split(' ')[1]
    cursor.execute("SELECT * FROM Mappings WHERE user_id = %s", userid)
    result = cursor.fetchall()
    temp = []
    if result:
        for i in result:
            temp.append({'mapid':i[0],'userid':i[1],'title':i[2],'description':i[3],'editdate':i[4].strftime('%Y-%m-%d %H:%M:%S')})
